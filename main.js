...
const debugTimeParam = new URLSearchParams(window.location.search).get("debugTime");
let forcedCycleTime = Number.isFinite(Number(debugTimeParam)) ? wrapTime(Number(debugTimeParam), LOOP_DURATION) : null;

const scene = new THREE.Scene();
scene.background = new THREE.Color(0xd8e9ef);
scene.fog = new THREE.FogExp2(0xd8e9ef, 0.0115);

const camera = new THREE.PerspectiveCamera(50, window.innerWidth / window.innerHeight, 0.1, 320);
camera.position.set(0, 40, 104);
...
class CreatureSilhouette {
  ...
  update(cycleTime) {
    ...
    const easedApproach = easeInOutCubic(approach);
    const squeeze = 1 - smoothstep(0.82, 1, approach);
    const x = this.side * (START_X * (1 - easedApproach) + this.startOffset * squeeze);
    ...
    const humanBlend = smoothstep(IMPACT_TIME - 4.2, IMPACT_TIME - 0.95, cycleTime);
  ...
  getAnimalShape(cycleTime) {
    ...
    const phase = fract(cycleTime / 5.9 + this.morphOffset) * species.length;
...
class InkBurstSystem {
  ...
  update(cycleTime) {
    const localTime = cycleTime - (IMPACT_TIME - 0.15);
...
function animate() {
  ...
  const birthFade = smoothstep(0.2, 1.8, cycleTime);
  const impactFade = 1 - smoothstep(IMPACT_TIME - 0.36, IMPACT_TIME + 0.18, cycleTime);
  silhouetteMaterial.opacity = birthFade * impactFade;
